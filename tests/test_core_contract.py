...
def _signed_10_sat_invoice():
    invoice = Bolt11(
        ...
        tags=Tags([
            Tag(TagChar.payment_hash, "33" * 32),
            Tag(TagChar.payment_secret, "44" * 32),
            Tag(TagChar.description, "ln-church-agent MPP test invoice"),
        ]),
    )
    return encode(invoice, private_key="02".zfill(64))
...
def test_execute_paid_action_compatibility():
    ...
    with patch.object(client, "execute_request", return_value={"ok": True}) as execute_request:
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            result = client.execute_paid_action("/api/agent/omikuji", {"asset": "SATS"})

    assert result == {"ok": True}
    execute_request.assert_called_once_with(
        "POST", "/api/agent/omikuji", {"asset": "SATS"}, None
    )
    assert len(caught) >= 1
    assert issubclass(caught[-1].category, DeprecationWarning)
    assert "execute_paid_action" in str(caught[-1].message)
...
def test_protocol_purity_mpp_headers(MockLNProvider):
    ...
    parsed = ParsedChallenge(
        scheme="MPP", network="Lightning", amount=10, asset="SATS",
        parameters={"invoice": _signed_10_sat_invoice(), "charge": "charge456"},
