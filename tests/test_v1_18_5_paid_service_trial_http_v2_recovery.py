"""Apply the unchanged finite witness/recovery obligations to saved v2 R."""
import json
...
from test_v1_18_5_paid_service_trial_http_v2_contract import wire_v2, V2TaskTransport as FakeTaskTransport, report_for_v2 as report_for, status_for_v2 as status_for
from test_v1_18_5_paid_service_trial_http_v2_purchase import lane_v2 as lane, wire_v2
...
def test_crash_after_reservation_recovers_saved_purchase_without_resend(wire,lane,tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(lane.http,'fetch',lambda *a,**k:crash() if k.get('payment_signature') else __import__('test_v1_18_5_paid_service_trial_http_v2_purchase').V2HTTP.fetch(lane.http,*a,**k))
...
def test_final_evaluation_cannot_revive_or_repurchase(wire,lane,state):
    from ln_church_agent.paid_service_trial_models import PaidServiceTrialSubmissionStatusV2 as PaidServiceTrialSubmissionStatus
    result=lane.executor.execute(wire.credential,journal=lane.journal)
...
def test_post_h_recovery_consumes_server_result_without_new_purchase(wire,lane,tmp_path,monkeypatch,scenario,outcome):
    ...
    assert set(sent) == {'schema_version','task_id','task_type','task_definition_version',
                         'task_definition_digest','terms_digest','execution_id','submission_id','purchase','request_digest','purchase_terms_digest'}
