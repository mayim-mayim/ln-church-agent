...
def test_v1_5_outcome_with_receipt():
    ...
    def receipt_matcher(response: dict, receipt: SettlementReceipt, context: ExecutionContext):
        # Public Receiptにはraw proofではなく一方向digestだけが残る。
        is_success = (
            receipt is not None
            and receipt.proof_reference == sha256_prefixed("dummy_proof")
        )
        return OutcomeSummary(is_success=is_success, observed_state="Cross-Verified")

    fixture = load_contract_fixture()
    client = configure_contract_clock(Payment402Client(), fixture)
    
    with patch("requests.request") as mock_req:
        # 1回目(402) -> 2回目(200) の一連の流れをモックする
        resp_402 = contract_response(fixture)
        resp_200 = success_response(fixture, {"status": "ok"})
        ...
        with patch.object(client, "_process_payment", return_value=("dummy_proof", "Lightning", None)):
            result = client.execute_detailed(
                ...
                outcome_matcher=receipt_matcher,
            )
