...
* **Sequential one-attempt targets**: Runs targets in Manifest order, durably writes `ATTEMPT_STARTED` before target I/O, never retries or resends an indeterminate target, and records typed unattempted results when a new target cannot start before T+5.
...
* **Hondo policy boundary**: Displays server-provided registration, reward, maximum-principal, and capacity values without hardcoding, deriving, decrementing, reserving, or re-aggregating them. Alpha A/B/N values are contract evidence and Hondo policy, not SDK calculation rules; generic canonical atomic amounts and internally consistent capacities remain valid.
* **No-payment transport**: Uses a dedicated fixed-route transport with public-IP DNS validation and pinning, preserved TLS hostname verification, bounded requests and responses, no redirects or ambient credentials, and fail-closed handling for unexpected HTTP 402 responses.
* **Claim credential safety**: Keeps the lease-bound claim token out of normal model serialization, logs, exceptions, and CLI output; the CLI uses create-exclusive private credential files and secret-free ambiguity tombstones.
* **Immutable Claim binding**: Retains the API-supplied Task Definition reference as public immutable metadata, never executable instructions, and binds each independent Claim credential to its authoritative Claim-time reward snapshot. The reward address remains only a payout destination, never the Agent principal or wallet-control proof.
* **Host-Agent discovery boundary**: Keeps open-web discovery with the Host Agent or another third-party runtime. The Task has no Task-level domain or parameters and carries empty `seed_urls`; the SDK neither crawls nor searches the target and adds no browser, JavaScript, login, form, scan, payment, mutation, or Task Offer registration capability.
* **Public-safe Observation contract**: Adds strict bounded schemas and stable RFC 8785 JCS submission identity. An actual HTTP 402 discovered surface is required, but subtype precision is not an eligibility gate, so an `unknown` 402 remains valid. Repeated domains, URLs, or known surfaces are not locally inferred to be reuse or failure.
* **Deterministic guided bridge**: Makes `submit_and_complete_domain_observation()` and `task submit-complete` the standard path from a validated Observation through Register and Completion. Completion IDs are derived only from the strictly verified Register receipt; callers neither interpret nor re-enter them.
* **Secret-free restart checkpoints**: Persists `REGISTER_PENDING` before Register and `REGISTERED` before Completion through a durable sink. Resume reuses the exact submission identity or skips an already verified Register, while the checkpoint remains non-authoritative restart metadata with no Claim token or usable credential.
* **Separated status scopes**: Keeps Offer `OPEN`, Claim-response `CLAIMED`, and the six public Submission statuses distinct; private pre-submission `EXPIRED` and `ABANDONED` states never validate on a public Submission resource.
* **Exact reward outcomes**: Maps public Submission status to the six reward states (`pending`, `not_eligible`, `approved_pending`, `paid`, `failed`, and `ambiguous`) and accepts only the finite status-specific failure codes, including `claim_task_or_observation_binding_mismatch`. Free-form, cross-status, and otherwise inconsistent outcome combinations fail closed.
* **Claim-specific result semantics**: Uses the existing Task, Submission, and Observation IDs plus the successful Claim's immutable Definition and reward snapshots for status, without Task-level domain equality. A possibly stale TaskGet advertised reward is never Claim authority, and the Definition remains metadata that the SDK neither downloads nor executes.
* **Task-detail Execution pages**: Retains Hondo-provided public-safe Execution summaries only on Task detail, paginated through that route's existing `limit` and opaque `cursor`. List items never acquire summaries, and the SDK neither reaggregates a page nor invents an Execution identifier.
* **Network-free Claim verification**: Provides local verification that binds a public Execution summary to a known validated Claim snapshot and its existing public identities. Structure-only Task-detail parsing is not presented as Claim-bound proof, and the verifier never uses TaskGet advertised reward or reevaluates Observation quality.
* **Durable Completion and bounded polling**: Completion 2xx records durable Submission/CompletionReport acceptance only—not Evaluation acceptance, Settlement, or payment. Bounded polling returns the last valid `pending` or `approved_pending` result as non-terminal when its attempt, exchange, or observation window ends.
